...
public class PaperPoolUtil {
    private static final Logger LOG = Logger.getLogger(PaperPoolUtil.class.getName());
    private static HashMap<Integer, ResearchPaper> idVsPaperDetails;
    private static int nextId = 1;
    private static int nextPaperId = 1;

    public PaperPoolUtil() {
        initialize();
    }
    ...
    public static HashMap<Integer, ResearchPaper> getLatestPaperDetailsBasedOnSubmitterId(Integer submitterId) {
        ...
        for (Integer id : paperDtls.keySet()) {
            if (!Objects.equals(paperDtls.get(id).getSubmitterId(), submitterId)) {
                paperDtls.remove(id);
            }
        }
        return paperDtls;
